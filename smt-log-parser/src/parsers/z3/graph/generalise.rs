...
impl Terms {
    pub fn generalise(&mut self, strings: &mut StringTable, terms: Vec<TermIdx>) -> Option<TermIdx> {
        fn check<T: Copy>(mut terms: impl Iterator<Item = T>, mut predicate: impl FnMut(T, T) -> bool) -> bool {
        ...
        loop {
            ...
            if check(next.iter(), |t1, t2| t1 == t2) {
                ...
                deref = unsafe {
                    std::mem::transmute::<Vec<&Term>, Vec<&'static Term>>(d)
                };
                check(deref.iter(), |t1, t2| t1.kind == t2.kind && t1.child_ids.len() == t2.child_ids.len())
            } {
                ...
                let tidx = self.new_synthetic_term(TermKind::Generalised, next.into_boxed_slice(), meaning);
                children.push(tidx);
            }

            let (mut deref, mut meaning, mut children) = stack.pop().unwrap();
            while deref[0].child_ids.len() == children.len() {
                let tidx = self.new_synthetic_term(deref[0].kind, children.into_boxed_slice(), meaning);
