...
struct TermDisplayContext<'a> {
    string_matchers: Vec<(Cow<'a, (Cow<'static, str>, Option<NonMaxU32>)>, Cow<'a, defns::TermDisplay>)>,
...
impl serde::Serialize for defns::TermDisplayContext {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        let self_ = TermDisplayContext {
            string_matchers: string_matchers.iter().map(|(k, v)| (Cow::Borrowed(k), Cow::Borrowed(v))).collect(),
...
impl<'de> serde::Deserialize<'de> for defns::TermDisplayContext {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        let string_matchers = self_.string_matchers.into_iter().map(|(k, v)| (k.into_owned(), v.into_owned())).collect();
