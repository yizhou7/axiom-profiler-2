...
use petgraph::{graph::{Edge, IndexType, Node}, EdgeType};
...
impl<K, V> MemDbgImpl for TiVec<K, V> where Vec<V>: MemDbgImpl {
    fn _mem_dbg_rec_on(
        &self,
        ...
        self.0.raw._mem_dbg_rec_on(writer, total_size, max_depth, prefix, is_last, flags)
    }
}
impl<K, V> MemSize for TiVec<K, V> where Vec<V>: MemSize {
...
impl<K: MemSize, V: MemSize> MemSize for FxHashMap<K, V> {
    fn mem_size(&self, flags: mem_dbg::SizeFlags) -> usize {
        ...
        self.0.iter().map(|(k ,v)| k.mem_size(flags) + v.mem_size(flags)).sum::<usize>()
    }
}

// StringTable

impl MemDbgImpl for StringTable where {}
impl MemSize for StringTable where {
    fn mem_size(&self, flags: mem_dbg::SizeFlags) -> usize {
        core::mem::size_of::<Self>() +
...
impl<T> MemDbgImpl for BoxSlice<T> where [T]: MemDbgImpl {
    fn _mem_dbg_rec_on(
        &self,
        writer: &mut impl core::fmt::Write,
        total_size: usize,
        ...
        self.0._mem_dbg_rec_on(writer, total_size, max_depth, prefix, is_last, flags)
    }
}
impl<T> MemSize for BoxSlice<T> where [T]: MemSize {
    fn mem_size(&self, flags: mem_dbg::SizeFlags) -> usize {
...
impl MemDbgImpl for TransitiveClosure where {}
impl MemSize for TransitiveClosure where {
    fn mem_size(&self, _flags: mem_dbg::SizeFlags) -> usize {
        core::mem::size_of::<Self>() + self.inner().iter().map(|r| r.serialized_size()).sum::<usize>()
    }
}
...
impl<N, E, Ty: EdgeType, Ix: IndexType> MemSize for Graph<N, E, Ty, Ix> where {
    fn mem_size(&self, flags: mem_dbg::SizeFlags) -> usize {
        ...
        core::mem::size_of::<Self>() + nodes * std::mem::size_of::<Node<N, Ix>>() + edges * std::mem::size_of::<Edge<E, Ix>>()
    }
}
