...
use petgraph::{Direction::{self, Outgoing, Incoming}, stable_graph::NodeIndex};
...
impl Component for SelectedNode {
    type Message = Action;
    type Properties = SelectedNodeProps;
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        let selected_inst = &ctx.props().selected_inst;
        let hide_node = { 
            let callback = ctx.props().action.clone();
